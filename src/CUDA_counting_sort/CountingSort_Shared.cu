...
__global__ void occurrence(DATA *A,DATA *C,int N,DATA val){
  //utilizzo la shared memory dinamica
...
int main(int argc,char ** argv){
  ...
  for(int i_occurr = min; i_occurr <= max; i_occurr++){
      ...
      occurrence<<<dimGrid,dimBlock,th_p_block*sizeof(DATA)>>>(devA,devC,N,i_occurr);
